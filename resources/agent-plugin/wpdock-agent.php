...
define('WPDOCK_AGENT_VERSION', '1.3.19');
...
function wpdock_agent_handle_request()
{
  ...
  switch ($action) {
    case 'ping':
      ...
      wpdock_sync_fast_upload_auth();
      wpdock_json_success(array_merge(
        array(
          'version' => WPDOCK_AGENT_VERSION,
          'fast_upload_url' => plugins_url('wpdock-fast-upload.php', __FILE__),
          'fast_upload_max_chunk_bytes' => wpdock_fast_upload_max_chunk_bytes()),
        wpdock_temp_status()
      ));
      break;

    case 'fast_upload_info':
      wpdock_sync_fast_upload_auth();
      wpdock_json_success(array(
        'url' => plugins_url('wpdock-fast-upload.php', __FILE__),
        'max_chunk_bytes' => wpdock_fast_upload_max_chunk_bytes(),
        'version' => 1));
      break;
...
function wpdock_register_token_endpoint(WP_REST_Request $request): WP_REST_Response
{
  ...
  wpdock_write_fast_upload_auth($token);
  return new WP_REST_Response(array('success' => true));
}

function wpdock_fast_upload_auth_file(): string
{
  return WPDOCK_TEMP_DIR . '/fast-upload-auth.php';
}

function wpdock_write_fast_upload_auth(string $token): bool
{
  ...
  return file_put_contents(wpdock_fast_upload_auth_file() . '.tmp', $payload, LOCK_EX) !== false
    && @rename(wpdock_fast_upload_auth_file() . '.tmp', wpdock_fast_upload_auth_file());
}

function wpdock_sync_fast_upload_auth(): void
{
  $token = (string) get_transient('wpdock_agent_token');
  if ($token !== '') {
    wpdock_write_fast_upload_auth($token);
  }
}

function wpdock_fast_upload_ini_bytes(string $value): int
{
  $value = trim($value);
  if ($value === '') return 0;
  $number = (float) $value;
...
function wpdock_fast_upload_max_chunk_bytes(): int
{
  $upload = wpdock_fast_upload_ini_bytes((string) ini_get('upload_max_filesize'));
  $post = wpdock_fast_upload_ini_bytes((string) ini_get('post_max_size'));
  $limit = min($upload > 0 ? $upload : PHP_INT_MAX, $post > 0 ? $post : PHP_INT_MAX);
  if ($limit === PHP_INT_MAX) $limit = 8 * 1024 * 1024;
  return max(256 * 1024, min(8 * 1024 * 1024, $limit - 128 * 1024));
}
...
function wpdock_write_uploaded_chunk_at_offset(
  ...
  // Standalone fast-upload sessions and completed resume pointers.
  $fast_entries = array_merge(
    glob(WPDOCK_TEMP_DIR . '/fast-*') ?: array(),
    glob(WPDOCK_TEMP_DIR . '/fast-completed-*') ?: array()
  );
  foreach (array_unique($fast_entries) as $entry) {
    if (basename((string) $entry) === 'fast-upload-auth.php') {
      continue;
    }
    $before = is_dir($entry) ? wpdock_dir_size((string) $entry) : (int) @filesize($entry);
    $removed = is_dir($entry) ? wpdock_rmdir_recursive((string) $entry) : @unlink($entry);
    if ($removed) {
      $deleted_files++;
      $deleted_bytes += $before;
    }
...
function wpdock_temp_sweep(): void
{
  ...
  // Filesystem-only fast-upload sessions do not depend on wp-cron/DB state.
  $fast_sessions = glob(WPDOCK_TEMP_DIR . '/fast-*');
  if (is_array($fast_sessions)) {
    foreach ($fast_sessions as $entry) {
      if (basename((string) $entry) === 'fast-upload-auth.php') continue;
      if (is_dir($entry)) {
        $state = wpdock_read_json_array($entry . '/state.json');
        if (! is_array($state) || (int) ($state['expires'] ?? 0) < $now) {
          wpdock_rmdir_recursive((string) $entry);
        }
      } elseif (strpos(basename((string) $entry), 'fast-completed-') === 0) {
        $data = wpdock_read_json_array((string) $entry);
        if (! is_array($data) || (int) ($data['expires'] ?? 0) < $now) {
          @unlink($entry);
        }
      }
