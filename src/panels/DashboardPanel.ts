    ...
    switch (type) {
      ...
      case 'connectRemote':
        try {
          ...
          let agentInstallFailed: { message: string; diagnostic: unknown } | undefined;
          ...
          if (remote.autoInstallAgent) {
            try {
              this.postMessage({ type: 'progress', message: 'Проверка и настройка агента...' });
              await this.remoteService.checkAgent(remote.id, (msg) => {
                this.postMessage({ type: 'progress', message: msg });
              }, true);
              ...
              agentInstallFailed = { message: installErr.message, diagnostic };
            }
          }
          this.postMessage({ type: 'remoteConnected', remote, agentInstallFailed });
          await this.sendFullState();
        } catch (err: any) {
          this.postMessage({ type: 'error', message: err.message });
        }
      ...
      case 'scanSiteFolders':
        try {
          const scanDir = payload?.dir || this.siteManager.getSitesDirectory();
          const sites = this.siteManager.findAdoptableSites(payload?.dir);
          this.postMessage({ type: 'adoptableSitesFound', sites, scanDir });
        } catch (err: any) {
          this.postMessage({ type: 'error', message: err.message });
        }
        break;

      case 'browseSiteFolder':
        try {
          const folderUri = await vscode.window.showOpenDialog({
            canSelectMany: false,
            canSelectFiles: false,
            canSelectFolders: true,
            openLabel: 'Выбрать папку сайта',
          });
          if (!folderUri || folderUri.length === 0) {
            break;
          }
          const info = this.siteManager.inspectSiteFolder(folderUri[0].fsPath);
          if (!info) {
            this.postMessage({ type: 'error', message: 'В выбранной папке не найден wp-config.php, это не похоже на установку WordPress, либо она уже добавлена как сайт.' });
            break;
          }
          this.postMessage({ type: 'siteFolderPicked', info });
        } catch (err: any) {
          this.postMessage({ type: 'error', message: err.message });
        }
        break;

      case 'adoptSiteFolder':
        try {
          const newSite = await this.siteManager.adoptSiteFolder(
            payload.folderPath,
            payload.options,
            (msg) => this.postMessage({ type: 'progress', message: msg })
          );
      ...
      case 'installLocalAgent': {
        try {
          ...
          const agentZip = path.join(this.context.extensionPath, 'resources', 'wpdock-agent.zip');
          if (!fs.existsSync(agentZip)) {throw new Error('Архив wpdock-agent.zip не найден в ресурсах расширения');}
          ...
          const stagingRoot = path.join(pluginsDir, `.wpdock-agent-install-${Date.now()}`);
          const stagedDir = path.join(stagingRoot, 'wpdock-agent');
          const backupDir = path.join(pluginsDir, '.wpdock-agent-previous');
          fs.mkdirSync(pluginsDir, { recursive: true });
          await unzip(agentZip, stagingRoot);
          const stagedPhp = path.join(stagedDir, 'wpdock-agent.php');
          if (!fs.existsSync(stagedPhp)) {
            fs.rmSync(stagingRoot, { recursive: true, force: true });
            throw new Error('Архив агента не содержит wpdock-agent/wpdock-agent.php');
          }
          ...
          const destPhp = path.join(destDir, 'wpdock-agent.php');
          let version = '';
          try {
            version = fs.readFileSync(destPhp, 'utf8').match(/Version:\s*([0-9.]+)/i)?.[1] ?? '';
          ...
          Logger.log(`[DashboardPanel] installed/updated local wpdock-agent v${version || '?'} into "${site.name}" (activated=${activated})`);
