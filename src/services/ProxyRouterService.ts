 ...
 *
 *  1. HTTP proxy binds to a non-privileged port (preferred: 9080).
...
const LEGACY_PROXY_HTTP     = 8080;
const PROXY_PREFERRED_HTTP  = 9080;
...
export class ProxyRouterService {
  ...
  async start(options: { allowElevation?: boolean } = {}): Promise<void> {
    ...
    let httpPort: number;
    // Do not reuse the legacy 8080 target. This intentionally creates drift so
    // setupPortProxyAndHosts migrates the persisted Windows rule to port 9080.
    if (targets.http !== null && targets.http !== LEGACY_PROXY_HTTP && await this.isPortFree(targets.http)) {
  ...
  async setupPortProxyAndHosts(hostsEntries: Array<{ domain: string; ip: string }>): Promise<boolean> {
    ...
    const script = [
      `$ErrorActionPreference = 'Stop'`, ...portProxyCmds,
      `$entries = @(${psEntries})`,
  ...
  private async setupPortProxy(httpPort: number, httpsPort: number): Promise<void> {
    ...
    if (ok) {
      ...
      vscode.window.showWarningMessage(
        `WPDock: UAC отклонён: сайты работают с портом (:${this._port}), без HTTPS.`
      );
    }
  }
