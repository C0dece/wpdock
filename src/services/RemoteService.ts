...
let CHUNK_UPLOAD_CONCURRENCY = 4;
...
// Max concurrent single-file requests during incremental Pull (action=download_path).
// Each request re-bootstraps WordPress on the remote, so latency (not bandwidth) is
// the bottleneck on a shared host with hundreds of changed files, a small worker
// pool amortizes that fixed cost without the heavier per-connection load a sharded
// pack job puts on the host, so this can run a bit higher than MAX_PARALLEL_SHARDS.
const MAX_PARALLEL_INCREMENTAL_DOWNLOADS = 4;
...
export class RemoteService {
  ...
  private readonly fastUploadInfoCache = new Map<string, FastUploadInfo | null>();
  ...
  private normalizeConcurrency(value: number | undefined): number {
    const v = Number.isFinite(value) ? Number(value) : 4;
  ...
  async checkAgent(remoteId: string, onProgress?: (msg: string) => void, strictAutoInstall = false): Promise<{
    installed: boolean;
    active: boolean;
    responsive: boolean;
    version?: string;
    ...
    if (pingVersion) {
      ...
      const bundledVersion = await this.getBundledAgentVersion();
      if (bundledVersion && this.compareSemver(pingVersion, bundledVersion) < 0) {
        onProgress?.(`Агент ${pingVersion} найден. Обновление до ${bundledVersion}...`);
        const updated = await this.updateAgent(remoteId, onProgress);
        return { installed: true, active: true, responsive: true, version: updated.version ?? pingVersion };
      }
      ...
      if (/404|plugin_not_found|rest_no_route|Cannot find/i.test(message)) {
        if (remote.autoInstallAgent) {
          ...
          try {
            ...
            if (strictAutoInstall) {throw installErr;}
          }
        }
  ...
  async installAgent(remoteId: string, onProgress?: (msg: string) => void): Promise<void> {
    ...
    const currentVersion = await this.getAgentVersionIfResponsive(remote.url, appPassword);
    if (currentVersion) {
      await this.registerAgentToken(remote.url, remote.username, appPassword, token);
      remote.agentInstalled = true;
      remote.agentVersion = currentVersion;
      this.storage.saveRemote(remote);
      const bundledVersion = await this.getBundledAgentVersion();
      if (bundledVersion && this.compareSemver(currentVersion, bundledVersion) < 0) {
        onProgress?.(`Найден агент ${currentVersion}. Обновление до ${bundledVersion}...`);
        await this.updateAgent(remoteId, onProgress);
      } else {
        onProgress?.(`Агент готов${currentVersion ? ` · v${currentVersion}` : ''}.`);
      }
      Logger.log(`[RemoteService] installAgent verified remote=${remote.name} id=${remote.id} version=${currentVersion}`);
      return;
    }
...
`;
  ...
  private async runPackJobSharded(
    ...
    agentVersion?: string,
    pullPack?: PullPackTransferState,
    persistPullPack: () => void = () => {}): Promise<any> {
    const { unzipBuffer } = await import('../utils/zipUtils');
  ...
  private async runPackStreamResumable(
    ...
    pullPack?: PullPackTransferState,
    persistPullPack: () => void = () => {}): Promise<{ parts: number; bytes: number }> {
    const { unzipBuffer } = await import('../utils/zipUtils');
  ...
  private async recoverPackStream(
    ...
    onPartExtracted?: (nextSeq: number, partBytes: number) => Promise<void>): Promise<{ nextSeq: number; count: number; bytes: number; done: boolean; processed: number }> {
    const tag = `job=${jobId}${shard >= 0 ? ` shard=${shard}` : ''}`;
    let status: any;
    for (let attempt = 1; ; attempt++) {
      try {
  ...
  private async drainParts(
    ...
    onPartExtracted?: (nextSeq: number, partBytes: number) => Promise<void>): Promise<{ nextSeq: number; count: number; bytes: number }> {
    let count = 0;
    let bytes = 0;
    let seq = fromSeq;
  ...
  private async pullChangedFiles(
    ...
    if ((remote.fileTransferMode ?? 'agent') === 'ftp') {
      ...
      // Each entry is an independent, stateless download_path request (no shared
      // job cursor like pack_files), so a bounded worker pool can safely drain
      // them concurrently instead of one fixed ~2-3s round-trip at a time.
      let nextIndex = 0;
      let aborted = false;
      ...
      const concurrency = Math.min(changed.length, MAX_PARALLEL_INCREMENTAL_DOWNLOADS);
  ...
  private async fetchWithBasicAuth(
    ...
    const requestWithRedirects = async (
      ...
      const response = await fetch(currentUrl, {
        redirect: 'manual', ...currentOptions,
        headers: {
          Authorization: `Basic ${auth}`, ...(cookieHeader ? { Cookie: cookieHeader } : {}), ...(currentOptions?.headers ?? {}),
        },
      });
  ...
  private async pullUploadsDirect(
    ...
    basePct: number,
    rangePct: number,
    agentVersion?: string): Promise<{ total: number; downloaded: number; bytes: number; failed: number }> {
  ...
  private async streamDownload(
    ...
    appPassword: string,
    query: string,
    logLabel: string): Promise<{ buffer: Buffer; contentLength: number }> {
    const fetch = (await import('node-fetch')).default;
    const token = await this.getAgentToken(appPassword);
    const startTime = Date.now();
  ...
  private async downloadPartBySeq(
    ...
    seq: number,
    shard: number,
    expectedBytes: number): Promise<Buffer> {
  ...
  private async uploadToAgent(
    ...
    // Agent 1.3.19+: send the data through a standalone PHP gateway which does
    // not bootstrap WordPress or touch wp_options for every chunk. Discovery is
    // one regular agent request per remote and also refreshes the gateway token.
    const fastInfo = await this.getFastUploadInfo(siteUrl, appPassword);
    if (fastInfo) {
      try {
        return await this.uploadToFastGateway(
          fastInfo,
          appPassword,
          filePath,
          totalBytes,
        ...
        if (!this.isFastUploadUnavailableError(err)) {
          throw err;
        }
        Logger.log(
          `[RemoteService] fast upload gateway unavailable; falling back to WordPress transport ` +
          `file=${path.basename(filePath)} error=${this.formatShortError(err)}`
        );
        this.fastUploadInfoCache.set(this.normalizeBaseUrl(siteUrl), null);
      }
    }
  ...
  private async getFastUploadInfo(
    siteUrl: string,
    appPassword: string
  ): Promise<FastUploadInfo | undefined> {
    const key = this.normalizeBaseUrl(siteUrl);
    if (this.fastUploadInfoCache.has(key)) {
      return this.fastUploadInfoCache.get(key) ?? undefined;
    }
    try {
      const info = await this.agentRequest(siteUrl, appPassword, 'fast_upload_info', {});
      const url = String(info?.url ?? '').trim();
      if (!/^https?:\/\//i.test(url)) {
        this.fastUploadInfoCache.set(key, null);
        return undefined;
      }
      const maxChunkBytes = Math.max(
        MIN_CHUNK_UPLOAD_BYTES,
        Math.min(8 * 1024 * 1024, Number(info?.max_chunk_bytes) || 1024 * 1024)
      );
      const result = { url, maxChunkBytes };
      this.fastUploadInfoCache.set(key, result);
      Logger.log(
        `[RemoteService] fast upload gateway discovered url=${url} ` +
        `maxChunk=${this.formatBytes(maxChunkBytes)}`
      );
      return result;
    } catch (err) {
      Logger.log(`[RemoteService] fast upload discovery unavailable: ${this.formatShortError(err)}`);
      this.fastUploadInfoCache.set(key, null);
      return undefined;
    }
  }

  private async uploadToFastGateway(
    info: FastUploadInfo,
    appPassword: string,
    filePath: string,
    totalBytes: number,
    ...
    // Use the largest body accepted by PHP (up to 8 MB). The gateway removes
    // WordPress bootstrap overhead, so a few large requests can saturate the
    // connection without exhausting the site's WordPress worker pool.
    let chunkSize = Math.max(
      MIN_CHUNK_UPLOAD_BYTES,
      Math.min(info.maxChunkBytes, 8 * 1024 * 1024)
    );
    let sessionAttempt = 0;

    for (;;) {
      try {
        return await this.runFastUploadSession(
          info.url,
          token,
          filePath,
          totalBytes,
        ...
        if (this.isChunkSizeRejection(err) && chunkSize > MIN_CHUNK_UPLOAD_BYTES) {
          chunkSize = Math.max(MIN_CHUNK_UPLOAD_BYTES, Math.floor(chunkSize / 2));
          sessionAttempt = 0;
          Logger.log(
            `[RemoteService] fast upload reducing chunk size to ${this.formatBytes(chunkSize)} ` +
            `after host payload rejection`
          );
          continue;
        }
        if (this.isRetryableUploadError(err) && sessionAttempt < UPLOAD_SESSION_RETRY_COUNT) {
          sessionAttempt++;
          this.reduceAdaptiveUploadConcurrency(`fast upload ${this.formatShortError(err)}`);
          Logger.log(
            `[RemoteService] fast upload resuming session attempt=${sessionAttempt}/${UPLOAD_SESSION_RETRY_COUNT} ` +
            `error=${this.formatShortError(err)}`
          );
          continue;
        }
        throw err;
  ...
  private async runFastUploadSession(
    gatewayUrl: string,
    token: string,
    filePath: string,
    totalBytes: number,
    ...
    const workerCount = Math.min(this.getEffectiveUploadConcurrency(), totalChunks);
    const init = await this.fastGatewayJsonRequest(gatewayUrl, token, 'init', {
      filename,
    ...
    if (completedToken) {
      Logger.log(`[RemoteService] fast upload RESUME completed file=${filename} token=${completedToken}`);
      onProgress?.(totalBytes, totalBytes);
      return completedToken;
    }
    const uploadId = String(init?.upload_id ?? '');
    if (!uploadId) {
      throw new Error('Fast upload gateway did not return upload_id');
    }
    ...
    for (const index of received) {
      uploadedBytes += Math.min(chunkSize, Math.max(0, totalBytes - index * chunkSize));
    }
    if (received.size > 0) {
      Logger.log(
        `[RemoteService] fast upload RESUME id=${uploadId} received=${received.size}/${totalChunks} ` +
        `uploaded=${this.formatBytes(uploadedBytes)}`
      );
      onProgress?.(Math.min(uploadedBytes, totalBytes), totalBytes);
    } else {
      Logger.log(
        `[RemoteService] fast upload START id=${uploadId} file=${filename} ` +
        `chunks=${totalChunks} chunkSize=${this.formatBytes(chunkSize)} concurrency=${workerCount}`
      );
    }
    ...
    try {
      const workers = Array.from({ length: workerCount }, async () => {
        while (nextIndex < totalChunks && !aborted) {
          ...
          const expected = Math.min(chunkSize, totalBytes - start);
          const buffer = Buffer.allocUnsafe(expected);
          const read = await fileHandle.read(buffer, 0, expected, start);
          const chunk = read.bytesRead === expected ? buffer : buffer.subarray(0, read.bytesRead);
          try {
            const retries = await this.uploadFastGatewayChunk(
              gatewayUrl,
              token,
              uploadId,
              index,
            ...
            if (retries > 0) {
              this.reduceAdaptiveUploadConcurrency(`fast chunk ${index} retries=${retries}`);
            }
          } catch (err) {
            aborted = true;
            throw err;
          }
          uploadedBytes += read.bytesRead;
          onProgress?.(Math.min(uploadedBytes, totalBytes), totalBytes);
        }
      });
    ...
    const finalized = await this.fastGatewayJsonRequest(gatewayUrl, token, 'finalize', {
      upload_id: uploadId,
    }, AGENT_UPLOAD_CONTROL_TIMEOUT_MS);
    const fileToken = String(finalized?.file_token ?? '');
    if (!fileToken) {
      throw new Error('Fast upload gateway did not return file token');
    }
    Logger.log(`[RemoteService] fast upload SUCCESS file=${filename} token=${fileToken}`);
    onProgress?.(totalBytes, totalBytes);
    return fileToken;
  }

  private async fastGatewayJsonRequest(
    gatewayUrl: string,
    token: string,
    action: string,
    body: object,
    timeoutMs = AGENT_UPLOAD_CONTROL_TIMEOUT_MS
  ): Promise<any> {
    const fetch = (await import('node-fetch')).default;
    ...
    try {
      ...
      throw new Error(`Fast upload invalid response: ${res.status} ${text.slice(0, 300)}`);
    }
    if (!res.ok || !data?.success) {
      throw new Error(`Fast upload error ${res.status}: ${data?.message ?? text.slice(0, 300)}`);
    }
    return data.data;
  }

  private async uploadFastGatewayChunk(
    gatewayUrl: string,
    token: string,
    uploadId: string,
    chunkIndex: number,
    ...
    for (let attempt = 1; attempt <= UPLOAD_RETRY_COUNT; attempt++) {
      const upload = this.createUploadController(
        `fast chunk #${chunkIndex} ${filename}`,
      ...
      try {
        ...
        form.append(
          'chunk',
          this.streamWithProgress(this.bufferToStream(chunk), () => upload.arm()),
          {
        ...
        if (!res.ok || !data?.success) {
          throw new Error(`Fast upload chunk error ${res.status}: ${data?.message ?? text.slice(0, 300)}`);
        }
        return attempt - 1;
      } catch (err: any) {
        upload.clear();
        ...
        if (
          ...
          this.isChunkSizeRejection(err) ||
          this.isUploadStorageError(err)
        ) {
          throw err;
        }
        Logger.log(
          `[RemoteService] fast upload chunk RETRY index=${chunkIndex} ` +
          `attempt=${attempt}/${UPLOAD_RETRY_COUNT} error=${err.message}`
        );
        await new Promise((resolve) => setTimeout(resolve, UPLOAD_RETRY_DELAY_MS * attempt));
      }
    }
    throw lastError ?? new Error('Fast upload chunk failed');
  }

  private isFastUploadUnavailableError(error: unknown): boolean {
    const raw = String((error as any)?.message ?? error ?? '').toLowerCase();
    return (
      raw.includes('fast upload invalid response: 404') ||
      raw.includes('fast upload error 404') ||
      raw.includes('fast upload invalid response: 403') ||
      raw.includes('fast upload error 403') ||
      raw.includes('unknown action')
    );
  }
      ...
      const workers = Array.from({ length: workerCount }, async () => {
        while (nextChunkIndex < totalChunks && !aborted) {
          ...
          try {
            ...
            // Learn from the first stalled/502 request instead of waiting until
            // the whole large archive part finishes. The cap is used by the next
            // resumed session/part; concurrent failures may quickly drive 4→2→1.
            if (retriesUsed > 0) {
              this.reduceAdaptiveUploadConcurrency(
                `chunk ${currentIndex} retries=${retriesUsed} file=${filename}`
              );
            }
          } catch (err) {
            aborted = true;
  ...
  private async pushSplitArchives(
    ...
    let pushState = transferState.pushFiles?.planKey === planKey && transferState.pushFiles.mode === 'split'
      ? transferState.pushFiles
      : undefined;
    // Matching state means this is a retry even if no complete ZIP part was
    // extracted yet: the server may hold a partially uploaded current part.
    const isResumeAttempt = pushState !== undefined;
    if (!pushState) {
    ...
    // Остатки старых планов (upload-*.zip и chunks-* сессии живут до 24 ч)
    // способны удвоить требуемое место. Очищаем их только для действительно
    // нового плана. У совпавшего resume-плана может не быть завершённой части,
    // но текущая ZIP-часть уже частично лежать на сервере.
    if (completedParts.size === 0 && !isResumeAttempt) {
      await this.cleanupUploadLeftovers(siteUrl, appPassword, 'fresh split push');
    } else if (completedParts.size === 0 && isResumeAttempt) {
      Logger.log('[PUSH-SPLIT] resume attempt has no verified completed parts; preserving partial upload sessions');
    }
