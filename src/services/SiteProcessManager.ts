...
const LEGACY_SITE_HTTP_PORT = 8080;
...
export class SiteProcessManager {
  ...
  async startSite(site: WPSite): Promise<void> {
    ...
    // Site ports are persisted, but another VS Code window may have started a
    // different site on the same port since this site was last stopped. Nginx
    // can then exit while waitForPort sees the *other* server and falsely marks
    // this start as successful. Re-resolve the port immediately before spawn.
    const requestedPort = site.port === LEGACY_SITE_HTTP_PORT
      ? PREFERRED_SITE_HTTP_PORT
      : site.port;
    site.port = await this.pickFreePort(requestedPort);
    if (site.port !== requestedPort) {
      Logger.log(`[site] Порт ${requestedPort} занят, для «${site.name}» выбран ${site.port}`);
    }
...
`;
  ...
  private getSitePhpIni(site: WPSite): string {
    ...
    const lines = [
      ...
      `sendmail_from = ${sendmailFrom}`, ...(os.platform() === 'win32' && mailPort ? [
        `SMTP = 127.0.0.1`,
        `smtp_port = ${mailPort}`,
      ] : []), ...(sendmailShimPath ? [
