export interface WPSite {
  ...
  /** User intent: automatically restore this site after VS Code reload/restart. */
  autoStart?: boolean;
...
/** A folder found on disk that looks like an existing WordPress install not yet tracked by WPDock. */
export interface AdoptableSiteInfo {
  /** Absolute path to the site folder (becomes `WPSite.path`). */
  folderPath: string;
  /** Folder name, default site name. */
  name: string;
  /** True if wp-config.php sits directly in the folder root and needs migrating into `public/`. */
  needsMigration: boolean;
  /** Domain recovered from WP_HOME/WP_SITEURL in wp-config.php, if present. */
  domain?: string;
  /** DB name recovered from wp-config.php, if present. */
  dbName?: string;
}
