...
export default function App() {
    ...
    const handler = (event: MessageEvent) => {
      ...
      switch (msg.type) {
        ...
        case 'remoteConnected':
          ...
          if (msg.agentInstallFailed) {
            addToast('Подключение сохранено. Для первой установки агента нужен один ручной шаг в WordPress.', 'info');
            setAgentInstallDiagnostics((prev) => ({ ...prev, [msg.remote.id]: msg.agentInstallFailed.diagnostic ?? null }));
            navigate({ name: 'manual-agent', remoteId: msg.remote.id });
          } else {
            navigate({ name: 'remote-detail', remoteId: msg.remote.id });
          }
          break;
        case 'remoteUpdated':
          hideGlobalProgress();
  ...
  const topNavItems: Array<{
    ...
    { key: 'remote', label: 'Удалённый', icon: '☁', route: { name: 'connect-remote' }, isActive: route.name === 'connect-remote' },
