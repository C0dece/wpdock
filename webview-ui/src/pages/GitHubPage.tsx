      ...
      <div className="detail-hero card">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div className="mini-item-subtitle">@{user.login} · публичных репозиториев: {user.publicRepos}</div>
          </div>
          <button className="btn btn-danger btn-sm" style={{ marginLeft: 'auto' }} onClick={handleDisconnect}>Отключить</button>
        </div>
        <div className="detail-summary-grid">
          <div className="overview-stat"><span className="overview-stat-value">{repos.length}</span><span className="overview-stat-label">загружено</span></div>
          <div className="overview-stat"><span className="overview-stat-value">{sites.length}</span><span className="overview-stat-label">локальных сайтов</span></div>
          <div className="overview-stat"><span className="overview-stat-value">{siteId || linkSiteId ? 'К сайту' : 'Свободный'}</span><span className="overview-stat-label">режим</span></div>
          <div className="overview-stat"><span className="overview-stat-value">{busy ? 'Загрузка' : 'Готово'}</span><span className="overview-stat-label">состояние</span></div>
        </div>
      </div>
                ...
                <div>
                  <div className="toolbar-wrap" style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <button type="button" className="link-button mini-item-title" onClick={() => vscode.postMessage({ type: 'openBrowser', payload: { url: repo.url } })}>
                        {repo.fullName}
                      </button>
                    ...
                    <div className="site-card-meta-row" style={{ marginBottom: 0 }}>
                      <span className={`badge ${repo.isPrivate ? 'badge-gray' : 'badge-green'}`}>{repo.isPrivate ? 'Приватный' : 'Публичный'}</span>
