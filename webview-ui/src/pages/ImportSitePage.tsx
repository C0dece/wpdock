...
export default function ImportSitePage({ navigate, onToast }: Props) {
  const [tab, setTab] = useState<'zip' | 'cloud' | 'folder'>('zip');
  ...
  const [adoptableSites, setAdoptableSites] = useState<AdoptableSiteInfo[]>([]);
  const [selectedFolder, setSelectedFolder] = useState<AdoptableSiteInfo | null>(null);
  const [folderSiteName, setFolderSiteName] = useState('');
  const [folderPhpVersion, setFolderPhpVersion] = useState('8.2');
  ...
  React.useEffect(() => {
    const handler = (e: MessageEvent) => {
      ...
      if (msg.type === 'adoptableSitesFound') {
        setAdoptableSites(msg.sites ?? []);
        setScanDir(msg.scanDir ?? '');
        setScanning(false);
        setScanned(true);
      }
      if (msg.type === 'siteFolderPicked') {
        setSelectedFolder(msg.info ?? null);
        setFolderSiteName(msg.info?.name ?? '');
      }
  ...
  const handleScanFolders = () => {
    ...
    vscode.postMessage({ type: 'scanSiteFolders' });
  };

  const handleBrowseFolder = () => {
    vscode.postMessage({ type: 'browseSiteFolder' });
  };

  const handlePickFromScan = (s: AdoptableSiteInfo) => {
    setSelectedFolder(s);
    setFolderSiteName(s.name);
  };

  const handleAdoptFolder = () => {
    if (!selectedFolder) return;
    if (!folderSiteName.trim()) return onToast('Введите имя сайта', 'error');
    setBusy(true);
    setProgressPercent(5);
    setProgressMsg('Подключение сайта...');
    vscode.postMessage({
      type: 'adoptSiteFolder',
      payload: {
        folderPath: selectedFolder.folderPath,
        options: { name: folderSiteName, phpVersion: folderPhpVersion },
      },
    });
  };
    ...
    <div className="page sidebar-detail-page">
      <div className="page-header">
        ...
        <div className="page-title-wrap">
          ...
          <div className="page-subtitle">{tab === 'zip' ? (zipAnalysis?.zipPath.split(/[\\/]/).pop() || 'Импорт локального ZIP') : tab === 'folder' ? 'Подключение существующей папки' : 'Импорт облачного бэкапа'}</div>
        </div>
      </div>

      <div className="detail-hero card">
        <div className="site-card-meta-row">
          <span className="badge badge-green">Импорт</span>
      ...
      <div className="tabs tabs-compact">
        {(['zip', 'cloud', 'folder'] as const).map((t) => (
          <button key={t} className={`tab-btn ${tab === t ? 'active' : ''}`} onClick={() => setTab(t)}>
            {t === 'zip' ? '↙ ZIP' : t === 'cloud' ? '☁ Облако' : '📁 Папка'}
          </button>
        ))}
      </div>
          ...
          {!selectedFolder ? (
            <>
              <div className="card">
                <div className="empty-state" style={{ padding: '20px 8px' }}>
                  <div className="icon">📁</div>
                  <h3>Подключить существующую папку</h3>
                  <p>Найдёт WordPress-установки в папке сайтов WPDock или укажите папку вручную.</p>
                  <div className="toolbar-wrap" style={{ justifyContent: 'center' }}>
                    <button className="btn btn-primary" onClick={handleScanFolders} disabled={scanning}>
                      {scanning ? '⏳ Сканирование...' : '🔍 Сканировать папку сайтов'}
                    </button>
                    <button className="btn btn-secondary" onClick={handleBrowseFolder} disabled={scanning}>
              ...
              {scanned && adoptableSites.length === 0 && (
                <div className="card">
                  <div className="empty-state" style={{ padding: '16px 8px' }}>
                    <p>В {scanDir || 'папке сайтов'} не найдено неподключённых WordPress-установок.</p>
                  </div>
                </div>
              )}

              {adoptableSites.length > 0 && (
                <div className="card">
                  <div className="card-header"><span className="card-title">Найдено сайтов ({adoptableSites.length})</span></div>
                  <div className="stack-sm remote-history-list">
                    {adoptableSites.map((s) => (
                      <button key={s.folderPath} className="sidebar-list-item" onClick={() => handlePickFromScan(s)}>
                        <div className="sidebar-list-main">
                          <div className="sidebar-list-title">{s.name}</div>
                          <div className="sidebar-list-subtitle">{s.domain || s.folderPath}{s.needsMigration ? ' · требует миграции' : ''}</div>
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            <>
              <div className="card">
                <div className="card-header"><span className="card-title">Выбранная папка</span></div>
                <div className="site-card-meta-row">
                  <span className="site-card-chip">{selectedFolder.folderPath}</span>
                  {selectedFolder.domain && <span className="site-card-chip">{selectedFolder.domain}</span>}
                  {selectedFolder.needsMigration && <span className="badge badge-orange">требует миграции</span>}
                </div>
                <div className="toolbar-wrap" style={{ marginTop: 10 }}>
                  <button className="btn btn-secondary btn-sm" onClick={() => setSelectedFolder(null)} disabled={busy}>Другая папка</button>
                </div>
              </div>
              <div className="card">
                <div className="card-header"><span className="card-title">Настройки сайта</span></div>
                <div className="form-row">
                  <div className="form-group">
                    <label className="form-label">Имя сайта *</label>
                    <input className="form-input" placeholder="Мой сайт" value={folderSiteName} onChange={(e) => setFolderSiteName(e.target.value)} disabled={busy} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Версия PHP</label>
                    <select className="form-input" value={folderPhpVersion} onChange={(e) => setFolderPhpVersion(e.target.value)} disabled={busy}>
                      <option value="8.3">PHP 8.3</option><option value="8.2">PHP 8.2</option><option value="8.1">PHP 8.1</option><option value="8.0">PHP 8.0</option><option value="7.4">PHP 7.4</option>
                    </select>
                  </div>
                </div>
              ...
              <div className="sticky-bottom-bar form-action-bar">
                <button className="btn btn-secondary" onClick={() => navigate({ name: 'home' })} disabled={busy}>Отмена</button>
                <button className="btn btn-primary" onClick={handleAdoptFolder} disabled={busy}>{busy ? 'Подключение...' : 'Подключить сайт'}</button>
              </div>
            </>
          )}
        </div>
