...
export default function ManualAgentPage({ remoteId, remotes, diagnostic, navigate, onToast }: Props) {
  ...
  useEffect(() => {
    if (!remote || remote.fileTransferMode === 'ftp') return;
    setChecking(true);
    vscode.postMessage({ type: 'checkRemoteAgent', payload: { remoteId } });
  }, [remoteId, remote?.id, remote?.fileTransferMode]);
    ...
    <div className="page sidebar-detail-page">
      <div className="page-header">
        ...
        <div className="page-title-wrap">
          <h1 className="page-title">WPDock Agent</h1>
      ...
      <div className="detail-hero card">
        <div className="site-card-meta-row">
          <span className={`badge ${remote.agentInstalled ? 'badge-green' : 'badge-yellow'}`}>{remote.agentInstalled ? 'Агент установлен' : 'Агент не установлен'}</span>
        ...
        <div className="section-copy">
          {remote.agentInstalled
            ? 'Агент проверяется автоматически при открытии страницы. Здесь можно обновить его или запустить диагностику.'
            : 'Сначала попробуйте автоматическую установку. Ручные шаги понадобятся только при ограничениях хостинга.'}
        </div>
      </div>
      ...
      {(!remote.agentInstalled || !lastStatus?.responsive) && <details className="card manual-steps">
        <summary className="manual-steps-summary">Ручная установка ZIP: только если автоустановка не сработала</summary>
      ...
      {lastStatus && !lastStatus.responsive && (
        <div className="card">
