...
type SectionKey = 'overview' | 'links' | 'history' | 'service';
...
export default function RemoteDetailPage({ remoteId, remotes, sites, navigate, onToast }: Props) {
  ...
  useEffect(() => {
    const handler = (event: MessageEvent) => {
      ...
      if ((msg.type === 'agentUpdated' || msg.type === 'agentUpdateFailed') && msg.remoteId === remoteId) {
  ...
  return (
    <div className="page sidebar-detail-page">
      ...
      <div className="detail-hero card">
        <div className="site-card-meta-row">
          <span className={`badge ${remote.fileTransferMode === 'ftp' || remote.agentInstalled ? 'badge-green' : 'badge-yellow'}`}>
            {remote.fileTransferMode === 'ftp' ? 'FTP' : (remote.agentInstalled ? 'Агент готов' : 'Агент не готов')}
          </span>
          ...
          {remote.preferCreateSiteOnPull && <span className="site-card-chip">Новый сайт при Pull</span>}
        </div>
        <div className="action-cluster">
          <button className="btn btn-primary" onClick={() => navigate({ name: 'sync', remoteId, direction: 'pull', localSiteId: primaryLinkedSite?.id, quickPull: Boolean(primaryLinkedSite?.id) })}>↓ Скачать изменения</button>
          ...
          <button className="btn btn-secondary" onClick={() => vscode.postMessage({ type: 'openBrowser', payload: { url: remote.url } })}>↗ Открыть сайт</button>
      ...
      {remote.fileTransferMode !== 'ftp' && (
        <div className={`card remote-agent-strip ${remote.agentInstalled ? '' : 'warning-card'}`}>
          <div>
            <div className="mini-item-title">{remote.agentInstalled ? `WPDock Agent готов${remote.agentVersion ? ` · v${remote.agentVersion}` : ''}` : 'Требуется настройка WPDock Agent'}</div>
            <div className="section-copy">{remote.agentInstalled ? 'Проверка токена выполняется автоматически. Обновление не требует входа в wp-admin.' : 'WPDock попробует установить, активировать и зарегистрировать агент автоматически.'}</div>
          </div>
          <div className="toolbar-wrap">
            {remote.agentInstalled && <button className="btn btn-secondary btn-sm" disabled={updatingAgent} onClick={() => { setUpdatingAgent(true); vscode.postMessage({ type: 'updateAgent', payload: { remoteId } }); }}>{updatingAgent ? 'Обновление...' : 'Обновить агент'}</button>}
            <button className={`btn btn-sm ${remote.agentInstalled ? 'btn-ghost' : 'btn-primary'}`} onClick={() => navigate({ name: 'manual-agent', remoteId })}>{remote.agentInstalled ? 'Проверка и диагностика' : 'Настроить агент'}</button>
          </div>
        </div>
      )}
      ...
      <AccordionSection title="История синхронизации" open={openSections.history} onToggle={() => toggleSection('history')}>
        <div className="card">
            <div className="card-header"><span className="card-title">История</span></div>
      ...
      <AccordionSection title="Обслуживание" open={openSections.service} onToggle={() => toggleSection('service')}>
        <div className="card">
          <div className="section-copy" style={{ marginBottom: 12 }}>Редкие и потенциально опасные действия. Используйте их только для восстановления подключения.</div>
          <div className="toolbar-wrap">
            {remote.fileTransferMode !== 'ftp' && <button className="btn btn-secondary btn-sm" disabled={cleaningUploads} onClick={() => { setCleaningUploads(true); vscode.postMessage({ type: 'cleanupRemoteUploads', payload: { remoteId } }); }}>{cleaningUploads ? 'Очистка...' : 'Очистить остатки Push'}</button>}
            {remote.fileTransferMode !== 'ftp' && <button className="btn btn-danger btn-sm" disabled={resetting} onClick={() => { setResetting(true); vscode.postMessage({ type: 'resetRemoteWp', payload: { remoteId } }); }}>{resetting ? 'Сброс...' : 'Сбросить WordPress'}</button>}
            <button className="btn btn-danger btn-sm" onClick={() => vscode.postMessage({ type: 'removeRemote', payload: { remoteId } })}>Удалить подключение</button>
          </div>
        </div>
      </AccordionSection>
