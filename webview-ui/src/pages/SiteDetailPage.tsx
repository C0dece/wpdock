...
type SectionKey = 'overview' | 'git' | 'deploy' | 'backup' | 'tools' | 'settings';
  ...
  useEffect(() => {
    const handler = (event: MessageEvent) => {
      ...
      if (msg.type === 'localAgentInstalled' && msg.siteId === siteId) {
        ...
        onToast(`WPDock Agent${ver} установлен или обновлён${msg.activated ? ' и активирован' : ''}`, 'success');
      }
    ...
    <div className={`page sidebar-detail-page ${projectMode ? 'project-site-page' : ''}`}>
      <div className="page-header">
        ...
        <div className="page-title-wrap">
          ...
          <div className="page-subtitle">{projectMode ? `WPDock Project · ${displayUrl}` : displayUrl}</div>
        </div>
      </div>

      <div className="detail-hero card">
        <div className="site-card-meta-row">
          <span className={`badge ${isRunning ? 'badge-green' : site.status === 'starting' ? 'badge-yellow' : site.status === 'error' ? 'badge-red' : 'badge-gray'}`}>{isRunning ? 'работает' : site.status === 'starting' ? 'запускается' : site.status === 'error' ? 'ошибка' : 'остановлен'}</span>
        ...
        <div className="action-cluster">
          {isRunning ? (
            <>
              ...
              <button className="btn btn-primary" onClick={() => vscode.postMessage({ type: 'openBrowser', payload: { url: siteUrl } })}>🌐 Открыть сайт</button>
              <button className="btn btn-secondary" onClick={() => vscode.postMessage({ type: 'autoLoginAdmin', payload: { siteId } })}>🔑 Админка</button>
              ...
              <button className="btn btn-ghost" onClick={() => vscode.postMessage({ type: 'stopSite', payload: { siteId } })}>⏹ Остановить</button>
            </>
          ) : (
            <>
      ...
      <AccordionSection title="Обзор" open={openSections.overview} onToggle={() => toggleSection('overview')}>
        <div className="detail-grid">
          <div className="card">
            ...
            {[
              ['Статус', isRunning ? 'работает' : site.status === 'starting' ? 'запускается' : site.status === 'error' ? 'ошибка' : 'остановлен'],
              ['URL', displayUrl],
              ['PHP', site.phpVersion],
      ...
      <AccordionSection title="Синхронизация и подключения" open={openSections.deploy} onToggle={() => toggleSection('deploy')}>
        <div className="card">
          <div className="card-header"><span className="card-title">Плагин WPDock Agent</span></div>
          <div className="section-copy">Установит или обновит WPDock Agent из ZIP, встроенного в расширение. На запущенном сайте плагин активируется автоматически.</div>
          <button className="btn btn-secondary btn-sm" onClick={() => vscode.postMessage({ type: 'installLocalAgent', payload: { siteId } })}>Установить / обновить агент</button>
        </div>
        <div className="card">
      ...
      <AccordionSection title="Инструменты" open={openSections.tools} onToggle={() => toggleSection('tools')}>
        <div className="card">
          <div className="card-header"><span className="card-title">Диагностика и разработка</span></div>
          <div className="section-copy">База данных, почта, журналы и командная строка нужны реже, поэтому собраны отдельно.</div>
          <div className="toolbar-wrap" style={{ marginTop: 10 }}>
            <button className="btn btn-secondary btn-sm" disabled={!isRunning} onClick={() => vscode.postMessage({ type: 'openDatabase', payload: { siteId } })}>🗄 База данных</button>
            <button className="btn btn-secondary btn-sm" disabled={!isRunning} onClick={() => vscode.postMessage({ type: 'openWpCliTerminal', payload: { siteId } })}>$ Терминал WP-CLI</button>
            <button className="btn btn-secondary btn-sm" onClick={() => vscode.postMessage({ type: 'openLocalMailFolder', payload: { siteId } })}>✉ Почта</button>
            <button className="btn btn-secondary btn-sm" onClick={() => vscode.postMessage({ type: 'openSiteLogs', payload: { siteId } })}>📜 Логи</button>
          </div>
        </div>
        <div className="card">
          <div className="card-header"><span className="card-title">Доступ и команда WP-CLI</span></div>
          ...
          <button className="btn btn-secondary btn-sm" style={{ marginTop: 10 }} onClick={() => vscode.postMessage({ type: 'getSiteCredentials', payload: { siteId } })}>{showCreds ? 'Обновить пароли' : 'Показать пароли'}</button>
