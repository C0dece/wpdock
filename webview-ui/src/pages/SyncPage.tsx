    ...
    <div className="page sidebar-detail-page">
      <div className="page-header">
        ...
        <div className="page-title-wrap">
          <h1 className="page-title">{isPull ? 'Скачать с сервера (Pull)' : 'Загрузить на сервер (Push)'}</h1>
      ...
      <div className="detail-hero card">
        <div className="site-card-meta-row">
          ...
          <span className="site-card-chip">{remote.fileTransferMode === 'ftp' ? 'FTP' : 'агент'}</span>
          <span className="site-card-chip">{syncMode === 'incremental' ? 'только изменения' : 'полностью'}</span>
          <span className="site-card-chip">{includeDb ? 'с базой данных' : 'только файлы'}</span>
          {isQuickPull && <span className="site-card-chip">быстрый режим</span>}
        </div>
      ...
      {!syncing && !done && (
        <div className="stack-sm">
          <div className="card">
            <div className="card-header"><span className="card-title">Параметры синхронизации</span></div>
            {isQuickPull ? (
              <>
                ...
                <div className="toolbar-wrap" style={{ marginBottom: 10 }}>
                  ...
                  <button type="button" className={`btn btn-sm ${syncMode === 'full' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setSyncMode('full')}>Перенести всё</button>
                </div>
              ...
              <>
                ...
                <div className="toolbar-wrap">
                  ...
                  >
                    Забыть состояние синхронизации
                  </button>
                </div>
              </>
            )}
          ...
          <div className="sticky-bottom-bar form-action-bar">
            {isQuickPull && !quickConfirm ? (
              <>
                ...
                <button className="btn btn-secondary" onClick={() => setIncludeDb(!includeDb)}>{includeDb ? 'С базой данных' : 'Без базы данных'}</button>
      ...
      {(syncing || done) && (
        <div className="stack-sm">
          <div className="card">
            <div className="card-header"><span className="card-title">{done ? 'Синхронизация завершена' : 'Идёт синхронизация'}</span></div>
            {steps.length > 0 && (
              <>
          ...
          {done && (
            <div className="sticky-bottom-bar form-action-bar">
              ...
              <button className="btn btn-secondary" onClick={() => navigate({ name: 'remote-detail', remoteId })}>К подключению</button>
            </div>
          )}
        </div>
      )}
